package com.netsuite.chinalocalization.common.control
...
public class CNDropdownList extends AbstractPopupWidget implements ICommonControl {
    private static final Logger log = LoggerFactory.getLogger(CNDropdownList.class);
    ...
    public CNDropdownList(GenericWebApp context, WebDriver webDriver, String suiteScriptID) {
    ...
    public CNDropdownList(GenericWebApp context, WebDriver webDriver, Locator parentLocator) {
    ...
    public void selectItem(String name) {
        if (name.equals(this.getValue())) {
            ...
            if (option != null) {
                if (name.equalsIgnoreCase(StringUtil.trim(option.getAsText()))) {
                    option.scrollToView();
                    option.click(false);
                }
            } else {
    ...
    public void selectItem2(String name) {
        if (name.equals(this.getValue())) {
            log.info(name + " already selected in dropdown " + this.widgetLocator);
            this.clickArrow(false);
        } else {
            ...
            String optionText=option.getAsText()
            ...
            if (option != null) {
                if (name.equalsIgnoreCase(StringUtil.trim(optionText))) {
                    option.scrollToView();
                    option.click(false);
                }
            } else {
    ...
    public List<String> getOptions() {
        ...
        LinkedHashMap<String, Locator> options = this.parseTableRowLocator("//div[@class='dropdownDiv' and contains(@style, 'visible')]/div[contains(@style,'block')]");
        ...
        if(items.size()==0){
            options = this.parseTableRowLocator("//div[@class='dropdownDiv' and contains(@style, 'visible')]/div");
            var3 = options.keySet().iterator();
    ...
    private void clickArrow(boolean expectPopup) {
        HtmlElementHandle parentElement = this.webDriver.getHtmlElementByLocator(this.widgetLocator);
        if (parentElement == null) {
            throw this.getExceptionFactory().createNSAppException(String.format("Element '%s' is not available!", this.widgetLocator));
        } else {
