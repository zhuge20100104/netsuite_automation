...
class SubLedgerClassficationTest extends ExtendReportAppTestSuite {
    ...
    def pathToTestDataFiles(){
        ...
        return [
                "enUS":"${dataFilesPath}extendreport\\data\\SubLedgerClassficationTest_en_US.json",
                "zhCN":"${dataFilesPath}extendreport\\data\\SubLedgerClassficationTest_zh_CN.json"
        ]
    }
    ...
    def checkExportedExcelFile(expResult) {
        ...
        for (int i=0; i<pdfContents.size(); i++){
            ...
            switch (index) {
                case 0:
                    ...
                    break
                case 2:
                    assertTrue("check ${line} has  subsidiary：${expPdf.subsidiary} ", line.contains(expPdf.subsidiary))
                    assertTrue("check ${line} has date ：${expPdf.period}", line.contains(expPdf.period))
                    assertTrue("check ${line} currency： ${expPdf.currency}", line.contains(expPdf.currency))
                    break
                case 3:
                    // check pdf account
                    if (i%2 == 1){
                    ...
                    expPdf.classification.each { key, value ->
                        if (count < 2){
                            assertTrue("check classification ${line} has ${value}", line.contains(value))
                            count ++
                        }
                        else {
                            count = 10
                ...
                case 4:
                    if (expPdf.classification.size() == 3){
                        assertTrue("check classification ${line} has ${expPdf.classification.class}", line.contains(expPdf.classification.class))
                    }
                    break
                default:
                    break
    ...
    void test_case_8_2_2() {
        initData()
        def expSearch = caseData.searchparm
        def expResult = caseData.ExcelData
    ...
    /**
     * @desc classification parameters -- export excel-- two parameters
     * @case 8.2.4
     *      1.Checked Use Account Number
     *      2.Checked ONLY SHOW LAST SUBACCOUNT
     *      3. Date Format: M/D/YYYY
    ...
    /**
     * @desc classification parameters -- export excel -- one parameter
     * @case 8.2.5
     *      1.Checked Use Account Number
     *      2.Checked ONLY SHOW LAST SUBACCOUNT
     *      3. Date Format: M/D/YYYY
